...
* __Authors__: Андрей Грабовой
* __Consultant__: Папай Иван (tg: [@papayiv_1503](https://t.me/papayiv_1503))
...
- **Данные**: Предлагается проводить вычислительный эксперимент на задаче классификации изображений, используя наборы данных
  ...
  - [FashionMNIST](https://pytorch.org/vision/0.20/generated/torchvision.datasets.FashionMNIST.html#torchvision.datasets.FashionMNIST)
  - [CIFAR10](https://pytorch.org/vision/0.20/generated/torchvision.datasets.CIFAR10.html#torchvision.datasets.CIFAR10)
- **Базовый алгоритм**: Опираться на явную запись $\mathbf{H}^{(k)} = \frac{1}{k}\sum\_{i=1}^k \mathbf{H}\_i$ и разность $\mathbf{H}\_{k+1} - \mathbf{H}^{(k)}$; не ограничиваться оценкой по треугольнику, а использовать разложение разности, ограниченность или концентрацию пер-объектных Гессианов, либо свойства спектра.
- **Решение**:
  - Записать разность $\mathbf{H}\_{k+1} - \frac{1}{k}\sum\_{i=1}^k \mathbf{H}_i$ в форме, удобной для оценки (например, через разность с «популяционным» средним или через суммы разностей пар)
  ...
  - Сравнить новую оценку с существующей $2 M\_{\mathbf{H}}$ и с поведением в эксперименте (оценка нормы разности Гессианов и метрики сходимости поверхности при разных $k$)
...
## Задача 214
* __Название__: Предсказание оптимальной стратегии генерации формального доказательства по формулировке задачи
* __Задача__: Специализированные модели генерации доказательств (DeepSeek-Prover-V2) поддерживают множество конфигураций вывода — режимы семплирования, параметры поиска, цепочки тактик — образующих пространство из 16 стратегий. Эмпирически установлено, что эффективность стратегии существенно зависит от типа задачи: стратегия, решающая 80% задач одного класса, может решать менее 10% задач другого. Полный перебор стратегий при фиксированном вычислительном бюджете неприемлем.
Предлагается построить классификатор, который по формулировке задачи на Lean 4 предсказывает подмножество стратегий, максимизирующее вероятность решения. Формально: дано множество задач $P$, множество стратегий $S = \{s_1, \ldots, s_{16}\}$ и матрица результатов $R \in \{0,1\}^{|P| \times |S|}$ (решена/не решена). Требуется обучить модель $f: P \to 2^S$, такую что при фиксированном бюджете $K$ попыток выбор стратегий из $f(p)$ даёт $\text{Pass@K}$ выше бейзлайна (равномерный выбор). В качестве признакового представления задачи рассмотреть: (1) эмбеддинги формулировки; (2) структурные признаки (глубина вложенности типов, число кванторов, зависимости от MathLib); (3) тематическую принадлежность (алгебра, анализ, комбинаторика и т.д.).

*Дополнительно* исследовать применимость модели для узлового выбора стратегии в tree search — когда доказательство декомпозируется на подцели, для каждой подцели стратегия выбирается независимо, построить алгоритм генерации дерева решения.

*Данные:* Бенчмарки MiniF2F (~250 задач) и PutnamBench (~600 задач), формализованные в Lean 4. Матрица результатов $R$ по всем 16 стратегиям предоставляется куратором (предгенерирована).

*Бейзлайн:* (1) Uniform — равномерный случайный выбор стратегии; (2) Round-robin — циклический перебор; (3) Majority — всегда выбирать стратегию с наибольшим средним pass rate.

*Требуется предложить:* метрики оценки качества выбора стратегии, учитывающие как точность предсказания, так и прогноз экономии ресурсов.

* __Авторы__:
	- Консультант: Андрей Халов (tg: [@andreykhalov](https://t.me/andreykhalov))
	- Эксперт: Ольга Муратовна Атаева


## Задача 215
* __Название__: Graph-RAG на графе знаний для генерации формальных доказательств
* __Задача__: LLM размера 7B при ограниченном бюджете семплирования показывают низкие результаты на задачах олимпиадного уровня (PutnamBench, IMO). Одна из ключевых причин — модель не имеет доступа к релевантным леммам, определениям и тактикам из MathLib (~200 000 утверждений) в момент генерации доказательства. Существующие retrieval-augmented подходы (ReProver) используют только векторный поиск по формулировкам, игнорируя графовую структуру зависимостей между теоремами.

Предлагается разработать Graph-RAG пайплайн, объединяющий два механизма поиска подсказок (premises): (1) dense retrieval — векторный поиск по эмбеддингам формулировок задачи и теорем MathLib; (2) графовый обход — по зависимостям в графе MathLib (используемые леммы, определения, пространства имён), позволяющий извлекать контекстуально связанные факты, не похожие на запрос текстуально, но необходимые для доказательства. Найденные подсказки добавляются в промпт модели. Обязательное требование — реализация гибридных векторно-графовых запросов.

Провести тестирование пайплайна с несколькими моделями и фреймворками: DeepSeek-Prover-V2, LeanDojo ReProver, InternLM2-Math. В качестве источников графовых данных использовать граф знаний на основе MathLib (доступ предоставляется) и векторное хранилище связанное сквозным индексом с графом знаний. Провести ablation study: вклад графовой компоненты vs. чисто векторного поиска на подмножестве данных из бенчмарков.

*Данные:* Подвыборки MiniF2F и PutnamBench (задачи, нерешённые бейзлайном); MathLib4 dependency graph; SciLib ontology; опционально — HERALD, ATLAS как дополнительные источники подсказок.

*Бейзлайн:* (1) Прямая генерация без RAG (DeepSeek-Prover-V2 7B, предоставляется); (2) Dense retrieval only (ReProver, предоставляется).

*Требуется предложить* метрики оценки качества поиска подсказок и итогового влияния аугментации на решаемость задач.
...
## Задача 216
* __Название__: Дистилляция мультимодального энкодера через контрастивное обучение в латентном пространстве старшей модели
* __Задача__: Крупные мультимодальные модели выравнивают представления нескольких модальностей в едином латентном пространстве, однако их размер исключает применение на edge-устройствах и в составе продуктивных пайплайнов. Стандартные подходы к дистилляции работают преимущественно для одной модальности и двух классов (позитив/негатив). Обобщение контрастивного обучения на произвольное число классов $C > 2$ с одновременной дистилляцией через старшую модель для $M \geq 3$ модальностей остаётся открытой задачей.
Предлагается: (1) зафиксировать предобученную модель-учитель $T$ и обучить компактный энкодер-ученик $S$ для $M \geq 3$ модальностей, минимизируя расхождение латентных представлений $d(z_T^{(m)}, z_S^{(m)})$ по каждой модальности $m$; (2) предложить обобщение supervised contrastive loss на случай $C > 2$ классов с дополнительным дистилляционным членом — выравнивание происходит одновременно между якорем и позитивами/негативами внутри batch и с целевыми эмбеддингами учителя; (3) показать, что alignment модальностей, достигнутый учителем, сохраняется у ученика после дистилляции, т.е. кросс-модальный retrieval не деградирует существенно при сжатии модели. Оценить робастность поиска при расширении пространства поиска.

Ключевой исследовательский вопрос — как трансформировать входящие в ученика векторы так, чтобы обучение происходило в латентном пространстве учителя, а ученик при этом сохранял собственную модальностно-специфичную архитектуру.

*Данные:* Мультимодальный датасет с $\geq 5$ модальностями (предоставляется куратором). Модель-учитель: предобученная мультимодальная модель (фиксирована, веса заморожены).

*Бейзлайн:* (1) Обучение компактного энкодера с нуля (без дистилляции); (2) MSE-дистилляция эмбеддингов (без контрастивного члена); (3) Стандартный InfoNCE/NT-Xent без дистилляции.

*Требуется предложить* метрики оценки качества дистилляции, учитывающие сохранение кросс-модального выравнивания и степень сжатия модели.
...
## Задача 217
* __Название__: Двунаправленный перевод математических утверждений между русским языком и Lean 4
* __Задача__: Формализация математических утверждений на языках доказательств является ключевым этапом в построении систем автоматического ассистента математика. Для англоязычных текстов существуют прототипы автоформализации (ProofNet, autoformalization benchmark), однако для русского языка инструменты прямого и обратного перевода практически отсутствуют. Это критично для интеграции формальных методов в русскоязычные сценарии ассистирования в доказательстве теорем.
Предлагается исследовать полный цикл перевода: NL-RU → FL → NL-RU, где NL-RU — математическое утверждение на русском языке, FL — формализация на Lean 4.

*Прямой перевод (NL-RU → FL).* Разработать или адаптировать метод перевода русскоязычных математических формулировок в утверждения на Lean 4. Исследовать как минимум два подхода: (a) цепочечный — NL-RU → NL-EN → FL через промежуточный перевод на английский; (b) прямой — NL-RU → FL. Корректность формализации верифицируется автоматически через Lean REPL: формализация считается синтаксически валидной тогда и только тогда, когда проходит type-check.

*Обратный перевод (FL → NL-RU).* Разработать метод генерации русскоязычного математического текста по формальному утверждению на Lean 4. Полученный текст должен быть математически точным, грамматически корректным и читаемым для носителя языка с математическим образованием.

*Метрики.* Предложить и обосновать метрики оценки семантической эквивалентности на каждом этапе цикла. В частности, исследовать применимость round-trip consistency (NL → FL → NL), автоматических метрик, экспертной оценки и верификации через Lean REPL. Выбор и обоснование метрик является частью задачи.

*Данные:* Датасет SciLibModal v1.

*Бейзлайн:* (1) GPT-4 / Claude в режиме zero-shot для NL-EN → FL; (2) Цепочка: машинный перевод (NL-RU → NL-EN) + LLM (NL-EN → FL); (3) Для FL → NL-RU: прямая генерация LLM по формальному утверждению.

*Требуется предложить* метрики оценки качества перевода в обоих направлениях, учитывающие как формальную корректность (type-check), так и сохранение математической семантики.
...
* __Авторы__:
  * Даниил Дорин (tg: [@danulkin](https://t.me/danulkin))
  * Консультант: Карпеев Глеб (tg: [@gkarp](https://t.me/gkarp))
