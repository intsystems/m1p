...
## Задача 145  (индустриальная)
  ...
  * **Описание проблемы**: Генеративные модели добились высокого качества генераций в общем домене. Однако, когда возникает запрос на генерацию специфичную объекта, в нашем случае человека, то модель не может сгенерировать человека с необходимой точностью. В этом задаче вам предлагается создать решение, которое будет способно генерировать изображения заданного человека в различных варияциях.
  * **Данные**: CelebA (датасет изображений знаменитостей в высоком разрешении)
  * **Литература**: 
	- [Создание портретов через механизм внимания](https://ip-adapter.github.io)
	- [Создание специальных токенов](https://dreambooth.github.io)
	- [Latent Diffusion Model](https://arxiv.org/pdf/2112.10752.pdf)
  * **Базовой алгоритм**: Базовый алгоритм состоит в обучении метода генерации через механизм адаптеров на модели  Stable Diffusion
